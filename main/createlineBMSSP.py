...
def create_line_bmssp(budget, memory, target, size, threshold, det, pre):
        ...
        for m in range(1,memory+1):
            for act in actions:
                for n in range(1,memory+1):
                    delta_vars.append(f'd_c{m}_o{s}_{act}_c{n} = Real(\'d_c{m}_o{s}_{act}_c{n}\')')
    
    #generate default delta functions (when no sensor observes - unknown state)
    for m in range(1,memory+1):
        for act in actions:
            for n in range(1,memory+1):
                delta_vars.append(f'd_c{m}_o_{act}_c{n} = Real(\'d_c{m}_o_{act}_c{n}\')')
                ...
                for act in actions:
                    for n in range(memory):
                        if act != 'l':
                            ...
                            equation += "+ "
                        equation += f'(1 + pi_s{left_next}_c{n}) * (y{s} * x_c{m}_o{s}_{act} * x_c{m}_o{s}_{act}_c{n} + (1 - ys{s}) * x_c{m}_o_{act} * x_c{m}_o_{act}_c{n}) \n'
                file.write(equation + ',\n')
    ...
    for s in sensor_states:
        for m in range(1,memory+1):
            for act in actions:
                ...
                theta_strategy_constraints.append(f'x_c{m}_o{s}_{act} == 0 or x_c{m}_o{s}_{act} == 1')
    ...
    for m in range(1,memory+1):
        for act in actions:
            ...
            theta_strategy_constraints.append(f'x_c{m}_o_{act} == 0 or x_c{m}_o_{act} == 1')       
